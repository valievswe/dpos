...
const BARCODE_BINARY_CANDIDATES = [
  "label.exe",
  "barcode.exe",
  "testbarcode.exe",
] as const;
...
const STORE_PHONE = "+998772215606";
...
const resolveBarcodeBinaryPath = (): { path: string; tried: string[] } => {
  ...
  return {
    path: tried[0] ?? getBinaryPaths(BARCODE_BINARY_CANDIDATES[0])[0],
    tried,
  };
};
...
const resolveReceiptBinaryPath = (): { path: string; tried: string[] } => {
  ...
  return {
    path: tried[0] ?? getBinaryPaths(RECEIPT_BINARY_CANDIDATES[0])[0],
    tried,
  };
};
...
const formatSom = (cents: number): string =>
...
export const PrinterService = {
  ...
  printLabel: (sku: string, productName: string) => {
    ...
    if (!fs.existsSync(exePath)) {
      console.error(
        `Label printer binary not found. Tried: ${tried.join(", ")}`,
      );
      return;
    }
    let barcode: string;
  ...
  printReceipt: (storeName: string, items: any[], total: string) => {
    ...
    const subtotal = Number.isFinite(totalValue)
    ...
    if (!fs.existsSync(exePath)) {
      console.error(
        `Receipt printer binary not found. Tried: ${tried.join(", ")}`,
      );
      return;
    }
  ...
  async printLabelByProduct(
    ...
    const db = getDB();
    const product = db
      .prepare("SELECT id, name, barcode FROM products WHERE id = ?")
  ...
  async printReceiptBySale(
    ...
    const sale = db
      .prepare(
        "SELECT id, subtotal_cents, discount_cents, total_cents, payment_method FROM sales WHERE id = ?",
      )
      .get(saleId) as any;
